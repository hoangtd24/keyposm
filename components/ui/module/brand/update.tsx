...
import {
    NEXT_PUBLIC_API_UPDATE_BRAND,
    NEXT_PUBLIC_UPLOAD_API
} from "@/config/api";

export interface DetailBrandProps {
    id: number;
    brandName: string;
...
export interface UpdateBrandProps {
    open: boolean;
    onClose: () => void;
    detail?: DetailBrandProps;
    onRefresh?: () => void;
}

export default function UpdateBrand({ open, onClose, detail, onRefresh }: UpdateBrandProps) {
    ...
    useEffect(() => {
        if (detail) {
            form.setValue("brandName", detail.brandName);
            form.setValue("brandDescription", detail.brandDescription);
            if (detail.brandLogo) {
                form.setValue("brandLogo", detail.brandLogo);
                setFileLogo(`${IMAGE_URI}/${detail.brandLogo}`);
            }
            // console.log(detail);
            if (detail.brandBackground) {

                form.setValue("brandBackground", detail.brandBackground);
                setFileBackground(`${IMAGE_URI}/${detail.brandBackground}`);
            }
            form.setValue("brandColor", detail.brandColor);
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        axiosWithHeaders("post", `${NEXT_PUBLIC_API_UPDATE_BRAND}`, objUpdate)
            .then((response) => {
        ...
        if (file) {
            ...
            await axiosWithHeadersUploadFile("post", NEXT_PUBLIC_UPLOAD_API, f)
                .then((response) => {
                    ...
                    // if (res.status === 200) {
                    //     form.setValue(field, res.data.data);
                    // }
                })
                .catch((err) => {
                    console.log(err);
                                        ...
                                        <FormItem>
                                            <FormControl>
                                                <div className="flex justify-center">
                                                    ...
                                                    <Input id="fileLogo" accept="image/*" disabled={(loadingFile || loadingFileBackground) ? true : false} type="file" className="hidden" onChange={(e) => onChangeFile(e, "brandLogo")} />
                                                </div>
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input accept="image/*" disabled={(loadingFile || loadingFileBackground) ? true : false} type="file" placeholder="Chọn tệp..." onChange={(e: any) => onChangeFile(e, "brandBackground")} />
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
                                    )}
