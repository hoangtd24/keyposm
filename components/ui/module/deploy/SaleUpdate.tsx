...
export default function SaleUpdateDeploy({
  deployId,
  setShowUpdate,
  showUpdate,
  setAlert,
  ...
  const handleUpdateDeploy: SubmitHandler<formValues> = async (
    ...
    const dataUpdate = {
      campaignId: deployDetail?.campaignId,
      posmId: deployDetail?.posmId,
      deployId: deployDetail?.id,
      location: `${location?.latitude},${location?.longitude}`,
      note: data.note,
      images: imgSrc.length > 0 ? res.data : [],
    };
    axiosWithHeaders("post", SALE_UPDATE_DEPLOY_API, dataUpdate)
      .then((response: any) => {
        if (response && response.status === enums.STATUS_RESPONSE_OK) {
        ...
        >
          <div className="p-6 relative z-10 overflow-x-hidden max-h-[90vh]">
            <DialogHeader className="relative z-10 mb-6">
              <DialogTitle>Cập nhật lắp đặt</DialogTitle>
            </DialogHeader>
            <div>
              <div>
