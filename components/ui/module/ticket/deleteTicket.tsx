...
export default function DeleteTicket({
  ...
  const handleUpdateDeploy: SubmitHandler<formValues> = async (
    ...
    setLoading(true);
    const dataDelete = {
      id: Number(query.ticketId),
      note: data.note,
    };
    axiosWithHeaders("post", NEXT_PUBLIC_DELETE_TICKET_API, dataDelete)
      .then((response: any) => {
        if (response && response.status === enums.STATUS_RESPONSE_OK) {
          ...
          if (status === enums.STATUS_RESPONSE_OK) {
            toast({
              title: "Thông báo",
              description: "Xóa sự cố thành công",
            });
            if (query.campaignId) {
              router.push(`/campaign/${query.campaignId}/ticket`);
            } else {
              router.push("/ticket");
            }
            onRefreshData();
