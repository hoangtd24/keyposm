...
export default function Detail({ params }: { params: { ticketId: string } }) {
  ...
  const userAssignTicket = (userId: number) => {
    ...
    if (existingUser && ticketDetail?.status >= 3) {
      return true;
    }
    return false;
  };
        ...
        <div className="bg-[#e5e5e5] p-4 lg:p-6 rounded flex flex-col gap-2">
          <div className="block lg:flex justify-between items-center">
            <div className="flex flex-col gap-2 flex-1">
              <div className="">
                <span className="break-words">Chiến dịch:</span>
                {loading ? (
              ...
              <div className="">
                <span className="break-words">Thương hiệu:</span>
                {loading ? (
              ...
              <div className="">
                <span className="break-words">Địa điểm:</span>
                {loading ? (
                  <Skeleton className="w-44 h-4 inline-block" />
                ) : (
              ...
              <div className="">
                <span className="break-words">Địa chỉ:</span>
                {loading ? (
                  <Skeleton className="w-44 h-4 inline-block" />
                ) : (
                  <span className=" break-words font-medium ml-2">
              ...
              <div className="">
                ...
                {loading ? (
                  ...
                  <span className="text-xs break-all font-mediums bg-[#f87674] px-2 py-0.5 rounded ml-2 text-white">
                    {ticketDetail?.statusName}
                  </span>
                ) : ticketDetail?.status === 2 ? (
                  <span className="break-all font-mediums text-xs px-2 py-0.5 rounded bg-[#6ba7ff] ml-2 text-white">
                    {ticketDetail?.statusName}
                  </span>
                ) : ticketDetail?.status === 3 ? (
                  <span className="break-all font-mediums text-xs px-2 py-0.5 rounded bg-[#ffb13f] ml-2 text-white">
                    {ticketDetail?.statusName}
                  </span>
                  ...
                  <span className="break-all font-mediums bg-[#ff1d19] text-sm px-2 py-0.5 rounded ml-2 text-white">
                    {ticketDetail?.statusName}
                  </span>
                ) : (
                  ""
          ...
          {ticketDetail?.totals?.length === 1 ? (
            ...
            <div>
              ...
              <table className="hidden lg:table w-full border-spacing-4 border-collapse border border-slate-400 mt-3">
                <thead>
                  <tr>
                    <th className="text-center border-2 border-current px-2 py-1 w-[25%]">
                      Tên POSM
                    </th>
                    <th className="text-center border-2 border-current px-2 py-1 w-[25%]">
                      Số lượng
                ...
                <tbody>
                  {ticketDetail?.totals?.map((item: any) => (
                    <tr key={item.id}>
                      <td className="px-2 py-1 text-center border-2 border-current">
                        {item.posmTypeName}
                      </td>
                      <td className="px-2 py-1 text-center border-2 border-current">
                        {
          ...
          <div className="flex justify-center gap-3 p-4">
            ...
            {ticketDetail?.status !== 5 &&
              (roleId === 1 || roleId === 2 || userAssignTicket(userId)) && (
